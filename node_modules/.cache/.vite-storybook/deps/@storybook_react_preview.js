...
// node_modules/@storybook/react/node_modules/@storybook/docs-tools/dist/index.mjs
...
var convert = (type) => {
  let { name, raw } = type, base2 = {};
  switch (typeof raw < "u" && (base2.raw = raw), type.name) {
    case "string":
    case "number":
    case "symbol":
    case "boolean":
      return { ...base2, name };
    case "Array":
      return { ...base2, name: "array", value: type.elements.map(convert) };
    case "signature":
      return { ...base2, ...convertSig(type) };
    case "union":
    case "intersection":
      return { ...base2, name, value: type.elements.map(convert) };
    default:
...
var convert2 = (type) => {
  ...
  switch (typeof raw < "u" && (base2.raw = raw), type.name) {
    case "literal":
      return { ...base2, name: "other", value: type.value };
    case "string":
    case "number":
    case "symbol":
    case "boolean":
      return { ...base2, name };
    case "Array":
      return { ...base2, name: "array", value: type.elements.map(convert2) };
    case "signature":
      return { ...base2, ...convertSig2(type) };
    case "union":
      return type.elements.every(isLiteral) ? { ...base2, name: "enum", value: type.elements.map(toEnumOption) } : { ...base2, name, value: type.elements.map(convert2) };
    case "intersection":
      return { ...base2, name, value: type.elements.map(convert2) };
    default:
      return { ...base2, name: "other", value: name };
  }
};
...
var convert3 = (type) => {
  let { name, raw, computed, value } = type, base2 = {};
  switch (typeof raw < "u" && (base2.raw = raw), name) {
    ...
    case "arrayOf":
    case "array":
      return { ...base2, name: "array", value: value && convert3(value) };
    case "object":
      return { ...base2, name };
    case "objectOf":
      return { ...base2, name, value: convert3(value) };
    case "shape":
    case "exact":
      let values = (0, import_mapValues.default)(value, (field) => convert3(field));
      return { ...base2, name: "object", value: values };
    case "union":
      return { ...base2, name: "union", value: value.map((v) => convert3(v)) };
    case "instanceOf":
    case "element":
    case "elementType":
    default: {
...
var enhanceArgTypes = (context) => {
  let { component, argTypes: userArgTypes, parameters: { docs = {} } } = context, { extractArgTypes: extractArgTypes2 } = docs, extractedArgTypes = extractArgTypes2 && component ? extractArgTypes2(component) : {};
...
function generateFuncSignature2(params, returns) {
...
function generateFunc(extractedProp) {
  ...
  return jsDocTags != null && (jsDocTags.params != null || jsDocTags.returns != null) ? createTypeDef({ name: "func", short: generateShortFuncSignature(jsDocTags.params, jsDocTags.returns), compact: null, full: generateFuncSignature2(jsDocTags.params, jsDocTags.returns) }) : createTypeDef({ name: "func", short: FUNCTION_CAPTION, compact: FUNCTION_CAPTION });
}
...
function generateUnion2(type, extractedProp) {
...
function generateType(type, extractedProp) {
  try {
    switch (type.name) {
      ...
      case "union":
        return generateUnion2(type, extractedProp);
      case "enum":
...
function createType4(extractedProp) {
...
function createDefaultValue4(defaultValue) {
  try {
...
function enhancePropTypesProp(extractedProp, rawDefaultProp) {
  let { propDef } = extractedProp, newtype = createType4(extractedProp);
  newtype != null && (propDef.type = newtype);
  let { defaultValue } = extractedProp.docgenInfo;
  if (defaultValue != null && defaultValue.value != null) {
    let newDefaultValue = createDefaultValue4(defaultValue.value);
...
var jsxDecorator = (storyFn, context) => {
  var _a, _b;
  let channel = import_preview_api2.addons.getChannel(), skip = skipJsxRender(context), jsx2 = "";
...
var applyDecorators = (storyFn, decorators2) => {
  ...
  return (0, import_preview_api3.defaultDecorateStory)(storyFn, reorderedDecorators);
};
